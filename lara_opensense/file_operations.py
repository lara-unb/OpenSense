...
import os
import pandas as pd
import lara_opensense.data_filters as data_filters
...
def filter_df(df, imu_labels, OpenSenseQuaternion=True):
    ...
    for label in imu_labels:
        ...
        print(f"tempo final: {df_imu_sf.time.iloc[-1]} {df_imu_sf.index[-1]}")

        imu_sampling_frequency = df_imu_sf.index[-1]/df_imu_sf.time.iloc[-1]
        print(imu_sampling_frequency)
        ...
        print(f"imu {label} {len(df_imu[label])}")
...
def json_to_sto(file_name, imu_labels, suffix='', filter=True, static=False):
    ...
    try:
        ...
        print("Erro ao ler JSON de captura")
        return
