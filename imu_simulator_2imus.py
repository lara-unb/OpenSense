...
def getSystemRotationMatrixes(df, calibrate_imu_position=True):
    ...
    if calibrate_imu_position:
        rot_shoulder_global_0 = getRotationMatrixFromEuler([0,0,-np.pi/2])
...
def show_animation(p_elbow, p_hand, time, interval):
    ...
    p_hand = p_hand-p_elbow

    # Inicializa os vetores
    q1 = ax.quiver(0, 0, 0, p_elbow[0,0], p_elbow[0,1], p_elbow[0,2], color='red', label='Braço')
    q2 = ax.quiver(p_elbow[0,0], p_elbow[0,1], p_elbow[0,2], p_hand[0,0], p_hand[0,1], p_hand[0,2], color='blue', label='Antebraço')
    ...
    def update(num):
        ...
        q2 = ax.quiver(p_elbow[num,0], p_elbow[num,1], p_elbow[num,2], p_hand[num,0], p_hand[num,1], p_hand[num,2], 
